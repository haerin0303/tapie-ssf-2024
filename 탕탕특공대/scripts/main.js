// 캔버스란 이름의 변수를 만드는데 캔버스라는 id 를 얻어와야 함
...
setInterval(()=>{
	enemies.map((enemy, i) => {
		//적의 체력이 0보다 작으면 킬을 1 올리고 enemies 배열에서 적을 지웁니다.
	})
...
function generateRandom(min = 0, max = canvas.width) {
	...
	// rand에 difference를 곱한뒤, 정수로 만들어서 rand변수에 저장하세요.
	rand += min;
	return rand;
}
...
function findDistance(missleX, missleY, enemyX, enemyY) {
	...
	//distance라는 변수를 만드는데 피타고라스 정리를 이용해 거리를 구하시오.
	return distance
}
...
function gameLoop() {
	...
	enemies.map((enemy) => {
		...
		if (newPlayer.health === 0) {
			isPlaying = false
			//gameCanvas 클래스를 가진 요소의 0번째에 디스플레이 속성을 없애십시오.
	...
	if (enemyDelay === 0 && enemies.length < 70) {
		if (minutes === 2) {
			enemies.push(new Enemy(generateRandom(), generateRandom(), /*채우기*/))
		} else if (minutes === 1) {
			enemies.push(new Enemy(generateRandom(), generateRandom(), /*채우기*/))
		}else  if (minutes === 0) {
			enemies.push(new Enemy(generateRandom(), generateRandom(), /*채우기*/))
		}
		//시간마다 적이 더 많이 생성되도록 설정하십시오
